...
/** <p>Architecture.SlideViewerComponent is a graphical component that can show slides.</p>
...
public class SlideViewerComponent extends JComponent implements SlideViewer {
	...
	public void paintComponent(Graphics g) {
		...
		g.setFont(labelFont);
		g.setColor(COLOR);
		g.drawString("FactoryMethodAndComposite.Slide " + (1 + presentation.getSlideNumber()) + " of " + presentation.getSize(), XPOS, YPOS);
		...
		for (SlideComponent item : slide.getSlideItems()) {
			if (!(item instanceof SlideItem)) {
				continue; // Skip if it's not a FactoryMethodAndComposite.SlideItem
			}
